...
class User extends KZ_Controller {
    ...
    function index() {
        ...
        $this->data['title'] = array('User','Daftar User');
    ...
    function _list_user() {
        ...
        foreach ($list as $items) {
            ...
            $status = ($items['status_user'] == '1') ? '<span class="label label-success label-white arrowed">AKTIF</span>' : '<span class="label label-danger label-white arrowed">TIDAK AKTIF</span>';
            $aksi = '<div class="action-buttons">
                        <a href="'. site_url($this->module .'/detail/'. encode($items['id_user'])) .'" class="tooltip-info btn btn-white btn-info btn-sm" data-rel="tooltip" title="Lihat Data">
                            <span class="blue"><i class="ace-icon fa fa-eye bigger-130"></i></span>
                        </a>
                        <a href="'. site_url($this->module .'/edit/'. encode($items['id_user'])) .'" class="tooltip-warning btn btn-white btn-warning btn-sm" data-rel="tooltip" title="Ubah Data">
                            <span class="orange"><i class="ace-icon fa fa-pencil-square-o bigger-130"></i></span>
                        </a>
                        <a href="#" name="'. encode($items['id_user']) .'" itemprop="'. ctk($items['fullname']) .'" id="delete-btn" class="tooltip-error btn btn-white btn-danger btn-sm" data-rel="tooltip" title="Hapus Data">
