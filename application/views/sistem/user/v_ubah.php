            ...
            <form id="validation-form" action="<?= site_url($action); ?>" name="form" class="form-horizontal" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <div class="col-xs-12 col-sm-3">
                        <div class="clearfix">
                            <select class="select2 width-100" name="group" id="group" data-placeholder="-------> Pilih Group User <-------">
                                <option value="">  </option>
                                <?php
                                foreach ($group['data'] as $val) {
                                    $selected = ($user['id_group'] == $val['id_group']) ? 'selected' : '';
                                    echo '<option value="'.$val['id_group'].'" '.$selected.'>'.$val['nama_group'].'</option>';
                                }
                                ?>
                            </select>
                        </div>
...
<script type="text/javascript">
    $('#validation-form').validate({
        ...
        messages: {
            nama: "Field Nama lengkap harus diisi",
            username: {
                required: "Field Username harus diisi",
                minlength: "Panjang field minimal 5 karakter"
            },
            group: "Pilih Group user dahulu",
            status: "Pilih status dahulu"
        },
