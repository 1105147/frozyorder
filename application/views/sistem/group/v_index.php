                        ...
                        <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                foreach ($group['data'] as $row) {
                                    ...
                                    <tr>
                                        <td><?= ctk($no); ?></td>
                                        <td><?= ctk($row['nama_group']); ?></td>
                                        <td><?= ctk($row['keterangan_group']); ?></td>
                                        <td><?= ($row['level'] == '1') ? '<span class="label label-info label-white arrowed">YA</span>' : '<span class="label label-warning label-white arrowed">TIDAK</span>' ?></td>
                                        <td nowrap>
                                            <div class="action-buttons">
                                                <a href="<?= site_url($module . '/edit/' . encode($row['id_group'])) ?>" class="tooltip-warning btn btn-white btn-warning btn-sm" data-rel="tooltip" title="Ubah Data">
                                                    <span class="orange"><i class="ace-icon fa fa-pencil-square-o bigger-130"></i></span>
                                                </a>
                                                <a href="#" name="<?= encode($row['id_group']) ?>" itemprop="<?= $row['nama_group'] ?>" id="delete-btn" class="tooltip-error btn btn-white btn-danger btn-sm" data-rel="tooltip" title="Hapus Data">
